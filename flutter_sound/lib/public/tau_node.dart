...
class TauTrack  {
  ...
  String? albumArtFile;
  /* ctor */  TauTrack( {this.title = 'A sound from Flutter Sound', this.author = 'τ', this.albumArtURL, this.albumArtAsset, this.albumArtFile}) {
...
class InputBuffer extends InputNode {
  ...
  Future<InputBuffer> toWave() async {
    ...
    Uint8List buffer = await tauHelper.pcmToWaveBuffer(inputBuffer: inputBuffer, codec: pcmCodec);
    return InputBuffer(buffer, codec: Pcm(AudioFormat.wav, sampleRate: pcmCodec.sampleRate, nbChannels: pcmCodec.nbChannels, endianness: pcmCodec.endianness, depth: pcmCodec.depth));
...
class InputFile extends InputNode {
  ...
  Future<InputFile> toWave() async {
    ...
    return InputFile(path, codec: Pcm(AudioFormat.wav, sampleRate: pcmCodec.sampleRate, nbChannels: pcmCodec.nbChannels, endianness: pcmCodec.endianness, depth: pcmCodec.depth));
  }
...
/* ctor */ DefaultInputDevice() { deprecatedAudioSource = AudioSource.defaultSource;}
}
...
class OutputFile extends OutputNode {
  ...
  Future<OutputFile> toWave() async {
    ...
    return OutputFile(path, codec: Pcm(AudioFormat.wav, sampleRate: pcmCodec.sampleRate, nbChannels: pcmCodec.nbChannels, endianness: pcmCodec.endianness, depth: pcmCodec.depth));
  }
